...
依目前 Chrome Web Store 表單與功能，勾選下列類型：

- Personally identifiable information：使用者選填並儲存在本機的預設收件 Email。
- Website content：使用者主動加入清單的商品名稱、圖片、價格、貨號、來源網站與商品連結。

不要勾選 Web history：RoomList 不建立或保存使用者造訪網頁的清單、造訪時間或完整瀏覽紀錄；商品連結是使用者主動加入清單時保存的網站內容。

不勾選：健康資料、財務和付款資訊、驗證資訊、個人通訊資料、位置資訊、網頁記錄及使用者活動。商品價格不是使用者的付款或金融帳戶資訊；Email 分享只會開啟使用者自己的郵件程式，RoomList 不讀取或保存郵件內容。
